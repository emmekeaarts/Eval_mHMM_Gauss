...
extracted_results_3st <- list(group_out_3st_emiss_mean = group_out_3st_emiss_mean,
...
Model_selection_3st <- data.frame(sim_iteration = 1:n_sim,
                                  true_m = true_m,
...
for(sc in 1:171){
  for(kc in 1:4){
    Model_selection_3st[(1 + (sc - 1) * n_sim * 4 + n_sim * (kc - 1)) : 
...
saveRDS(Model_selection_3st, file = "result_tables/Model_selection_3st.RDS")
...
for(sc in 1:171){
  Performance_emission_3st[(1 + cumsum_it[sc]) : (it[sc] + cumsum_it[sc]),9] <- unlist(group_out_3st_emiss_mean[[sc]]$median)
  Performance_emission_3st[(1 + cumsum_it[sc]) : (it[sc] + cumsum_it[sc]),10] <- unlist(means_true[[sc]])  
  Performance_emission_3st[(1 + cumsum_it[sc]) : (it[sc] + cumsum_it[sc]),11] <- unlist(group_out_3st_emiss_mean[[sc]]$quant_2.5)
  Performance_emission_3st[(1 + cumsum_it[sc]) : (it[sc] + cumsum_it[sc]),12] <- unlist(group_out_3st_emiss_mean[[sc]]$quant_97.5)
  Performance_emission_3st[(1 + cumsum_it[sc]) : (it[sc] + cumsum_it[sc]),13] <- unlist(group_out_3st_emiss_sd[[sc]]$median)
  Performance_emission_3st[(1 + cumsum_it[sc]) : (it[sc] + cumsum_it[sc]),14] <- 1
  Performance_emission_3st[(1 + cumsum_it[sc]) : (it[sc] + cumsum_it[sc]),15] <- unlist(group_out_3st_emiss_sd[[sc]]$quant_2.5)
  Performance_emission_3st[(1 + cumsum_it[sc]) : (it[sc] + cumsum_it[sc]),16] <- unlist(group_out_3st_emiss_sd[[sc]]$quant_97.5)
}
View(Performance_emission_3st)
...
Performance_gamma_3st <- data.frame(sim_iteration = 1:n_sim,
                                       true_m = true_m,
...
for(sc in 1:171){
  Performance_gamma_3st[ (1 + (sc-1) * it) : (sc * it),9] <- as.vector(group_out_3st_gamma_prob[[sc]]$median)
  Performance_gamma_3st[ (1 + (sc-1) * it) : (sc * it),11] <- as.vector(group_out_3st_gamma_prob[[sc]]$quant_2.5)
  Performance_gamma_3st[ (1 + (sc-1) * it) : (sc * it),12] <- as.vector(group_out_3st_gamma_prob[[sc]]$quant_97.5)
}

View(Performance_gamma_3st)
...
State_decoding_3st <- data.frame(sim_iteration = 1:n_sim,
                                  true_m = true_m,
...
for(sc in 1:171){
  State_decoding_3st[(1 + (sc-1) * 100) : (sc * 100),7] <- apply(inferred_states[[sc]][,-1] == true_states[[sc]][,-1], 2, sum) / (dim(true_states[[sc]][,-1])[1])
  ...
  State_decoding_3st[(1 + (sc-1) * 100) : (sc * 100),8] <- (agreements - chance_agreements) / ( matrix(apply(true_totals, 1, sum), ncol = 1) - chance_agreements)
  State_decoding_3st[(1 + (sc-1) * 100) : (sc * 100),9] <- apply(true_state_probs[[sc]][,-1] > 0.20, 2, sum) / (dim(true_states[[sc]][,-1])[1])
}

View(State_decoding_3st)
