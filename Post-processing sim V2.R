...
# Simulstion settings
settings2 <- read.csv("sim_scenarios_V2.csv")
true_m  <- 3
...
selected <- sort(c(1:32, 34:39, 42:69, 72, 76, 78, 79, 74, 80, 91, 93, 94, 97, 92, 95, 99:101, 103, 104, 106, 107))

files <- paste0("out_n", settings2[selected, 1], "_nt", settings2[selected, 2], "_ndep", settings2[selected, 3], "_", true_m, "st_KLD", settings2[selected, 4])
...
for(sc in 1:length(files)){
  out[[sc]] <- readRDS(paste0("out_V2/", files[sc], ".rds")) 
}
...
AICc_mean <- data.frame(true_m = 3,
                        n = settings2[selected, 1], 
                        n_t = settings2[selected, 2],
                        n_dep = settings2[selected, 3], 
                        KL_div = settings2[selected, 4], 
                        st1 = NA, 
                        st2 = NA, 
                        st3 = NA, 
                        st4 = NA)
