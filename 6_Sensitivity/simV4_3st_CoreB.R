...
settings <- read.csv("coreB_settings.csv")
...
out_file <- paste0("out_n", n, "_nt", n_t,"_KLD", KL_div, 
                   "_mu_sc", mu_sc, "_mu_K0", mu_K0_sc, "_tans", alph_unif, "_trans_K0", alph_K0_sc, "_varTauSig", var_tau_sig
                   )
out_file
input <- list(KL_div = KL_div, n_t = n_t, n = n, 
              mu_sc = mu_sc,         
              mu_K0_sc = mu_K0_sc,    
              trans_unif = alph_unif,  
              trans_K0_sc = alph_K0_sc, 
              var_tau_sig = var_tau_sig, 
              true_m = true_m, n_dep = n_dep, n_sim = n_sim, J = J, burn_in = burn_in)
...
manual_prior_emiss <- vector(mode='list', length=n_sim)
...
if(alph_unif == 0){
  ...
  } else if (var_tau_sig == 1){
    manual_prior_gamma <- prior_gamma(m = 3, 
                                      gamma_V = diag(0.01^2, nrow = 3-1),
                                      gamma_mu0 = list(prob_to_int(matrix(c(0.6, 0.2, 0.2), nrow = 1)),
                                                              prob_to_int(matrix(c(0.2, 0.6, 0.2), nrow = 1)),
                                                              prob_to_int(matrix(c(0.2, 0.2, 0.6), nrow = 1)))) 
  } else if (var_tau_sig == 2){
    manual_prior_gamma <- prior_gamma(m = 3, 
                                      gamma_V = diag(3^2, nrow = 3-1),
                                      gamma_mu0 = list(prob_to_int(matrix(c(0.6, 0.2, 0.2), nrow = 1)),
                                                       prob_to_int(matrix(c(0.2, 0.6, 0.2), nrow = 1)),
                                                       prob_to_int(matrix(c(0.2, 0.2, 0.6), nrow = 1)))) 
  ...
  if(alph_K0_sc == 0){
    manual_prior_gamma <- prior_gamma(m = 3, gamma_mu0 = list(matrix(rep(0, (3-1)), nrow = 1),
                                                                   matrix(rep(0, (3-1)), nrow = 1),
                                                                   matrix(rep(0, (3-1)), nrow = 1)))
  } else {
    manual_prior_gamma <- prior_gamma(m = 3, gamma_K0 = round(n * 0.2), gamma_mu0 = list(matrix(rep(0, (3-1)), nrow = 1),
                                                                   matrix(rep(0, (3-1)), nrow = 1),
                                                                   matrix(rep(0, (3-1)), nrow = 1)))
...
shrink_means <- function(means, factor = 0.5) {
  order_idx <- order(means)        # indices that would sort the vector
  sorted <- means[order_idx]       # sort to find the middle
  center <- sorted[2]              # middle value by magnitude
  shrunk_sorted <- center + (sorted - center) * factor  # shrink around center
  shrunk_sorted[order(order_idx)]  # restore original order
}

m <- 3
gen      <- list(m = m, n_dep = n_dep)
emiss_K0 <- if(mu_K0_sc == 0){
  rep(list(1), n_dep)
} else {
    rep(list(round(n * 0.2)), n_dep)
  }
emiss_V  <- if(var_tau_sig == 0){
  rep(list(rep(.1^2, m)), n_dep)
} else if(var_tau_sig == 1){
  rep(list(rep(.01^2, m)), n_dep)
} else if(var_tau_sig == 2){
  rep(list(rep(1^2, m)), n_dep)
}
...
for(s in 1:n_sim){
emiss_mu0 <- if(mu_sc == 0) { # default
    lapply(sapply(apply( 
      depmix_means[[3]][[s]], 1, list), matrix), matrix, nrow = 1)
  } else if(mu_sc == 1){ # all means equal
    lapply(lapply(apply(depmix_means[[3]][[s]], 1, mean), rep, each = 3), matrix, nrow = 1)
  } else if(mu_sc == 2){ # closer together means
    lapply(asplit(apply(depmix_means[[3]][[s]], 1, shrink_means), 2), matrix, nrow = 1)
  }  
...
saveRDS(out_1, file = paste0("out_S/", out_file, "_B.rds"))
